...
class BoardOutException(BoardException):
    def __str__(self):
            return "Ошибка, клетка вне игрового поля"

class BoardUsedException(BoardException):
    def __str__(self):
        return "Ошибка, в эту клетку уже стреляли"
...
class Ship:
    ...
    def dots(self):
        ...
        for i in range(self.l):
            cur_x = self.bow.x
            cur_y = self.bow.y
            if self.direction == 0: # вертикальное положение
                cur_x +=i
            elif self.direction == 1: # горизонтальное положение
                cur_y += i
...
class Board:
    def __init__(self,hid=False,size=6):
        ...
        self.hid = hid
        self.count = 0
        self.field = [["О"]* size for i in range(size)]
        self.buzy = []
        self.ships = []
